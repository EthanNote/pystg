...
import pygame
from pgzero import *
from pgzero import constants
from pgzero.game import DEFAULTICON, DISPLAY_FLAGS
...
class STGame:
    ...
    def reinit_screen(self):
        ...
        if w != self.width or h != self.height:
            ...
            pgzero.game.screen = self.screen  # KILL ME
            self.width = w
            self.height = h

        title = 'PY STG'  # getattr(self.mod, 'TITLE', 'Pygame Zero Game')
        if title != self.title:
    ...
    def run(self, handler_class):
        ...
        try:
            self.mainloop(handler_class)
        finally:
            pygame.display.quit()
            pygame.mixer.quit()

    def mainloop(self, handler_class):
        ...
        while True:
            ...
            #     update(dt)
            # print('on update')
            self.handler.onUpdate(dt)

            screen_change = self.reinit_screen()
            # if screen_change or pgzclock.fired or self.need_redraw:
                # draw()
